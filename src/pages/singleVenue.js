...
export default function SingleVenue() {
    ...
    //     try {
    //       const { data, error } = await supabase.from('screens').select().eq('eventOrganizationId', id);
    //       if (error) throw error;
    //       if (data) {
    //         setScreensData(data);
    //       }
    ...
    //     try {
    //       const { data: screenIdData, error: acreenIdDataError } = await supabase.from('screens').select('id').eq('eventOrganizationId', id);
